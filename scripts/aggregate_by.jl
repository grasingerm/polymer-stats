...
if length(ARGS) < 4
  println("usage: julia aggregate_by.jl <outdir> <indir> <param> <dielectric|polar> [<3D|2D>]");
  exit(1);
end
...
for datafile in datafiles
  ...
  @show pattern = string(datafile[1:value_start_index-1], "*", 
                         datafile[value_end_index:end])
