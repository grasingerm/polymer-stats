...
Ks = zip([1.0], [0.0])
kTs = [1.0;];
Fzs = vcat(collect(range(0.0, 1.0; step=0.01)), collect(range(1, 101; step=1)));
Fxs = [0.0;];
ns = Int[100; 200; 250; 1000];
bs = [1.0;];
...
pmap(case -> begin;
  ...
  if !isfile(outfile)
    ...
    command = `julia -O 3 ni_chain.jl -b $(case[:b]) --E0 $(case[:E0]) --K1 $(case[:K1]) --K2 $(case[:K2]) --kT $(case[:kT]) --Fz $(case[:Fz]) --Fx $(case[:Fx]) -n $(case[:n]) --num-steps 1000000 --num-inits 1 --step-adjust-scale 1.1 --step-adjust-ub 0.39 --step-adjust-lb 0.19 -v 2 --prefix $(joinpath(workdir, prefix(case)))`;
