...
cases = Any[];
E0s = range(1.5, 2.5; length=100);
Ks = zip([1.0; 0.0], [0.0; 1.0])
kTs = [1.0];
Fzs = vcat(collect(range(0.0, 1.0; step=0.5)), range(5.0, 10.0; step=2.5));
Fxs = [0.0];
ns = Int[100];
bs = [1.0];
...
pmap(case -> begin;
  for iter=1:10
    ...
    if !isfile(outfile)
      ...
      command = `julia -O 3 mcmc_eap_chain.jl --chain-type dielectric --energy-type noninteracting -b $(case[:b]) --E0 $(case[:E0]) --K1 $(case[:K1]) --K2 $(case[:K2]) --kT $(case[:kT]) --Fz $(case[:Fz]) --Fx $(case[:Fx]) -n $(case[:n]) --num-steps 100000 -v 2 --prefix $(joinpath(workdir, prefix(case)*"_iter-$iter"))`;
