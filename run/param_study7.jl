...
cases = Any[];
E0s = [0.1; 1.0; 2.5; 5.0; 10.0; 100.0];
...
pmap(case -> begin;
  ...
  if !isfile(outfile)
    ...
    command = `julia -O 3 mcmc_eap_chain.jl --chain-type dielectric --energy-type noninteracting -b $(case[:b]) --E0 $(case[:E0]) --K1 $(case[:K1]) --K2 $(case[:K2]) --kT $(case[:kT]) --Fz $(case[:Fz]) --Fx $(case[:Fx]) -n $(case[:n]) --num-steps 250000 -v 2 --prefix $(joinpath(workdir, prefix(case))) --umbrella-sampling`;
